...
public class InvestmentDataTzServiceImpl implements InvestmentDataTzService {
    ...
    public List<InvestmentDataTz> findOneHundred(InvestmentDataTz data, Pageable pageable) {
        if (pageable == null) {
            pageable = new PageRequest(0, 100);
        }
        Page<InvestmentDataTz> page = investmentDataTzRepository.findAll(new Specification<InvestmentDataTz>() {
            @Override
            public Predicate toPredicate(Root<InvestmentDataTz> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
    ...
    @Override
    @TargetDataSource(name = "chuangtou")
    public void save(List<InvestmentDataTz> news) {
        investmentDataTzRepository.save(news);
    }

    @Override
    @TargetDataSource(name = "chuangtou")
    public void delete(List<InvestmentDataTz> news) {
        investmentDataTzRepository.delete(news);
    }
